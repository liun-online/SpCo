...
def load_data(dataset, scope_flag, path="../dataset/"):
    print('Loading {} dataset...'.format(dataset))
    adj = sp.load_npz(path+dataset+"/adj.npz")
    print(adj.A[0,0])
    features = sp.load_npz(path+dataset+"/feat.npz")
    labels = np.load(path+dataset+"/label.npy")
    labels = encode_onehot(labels)
    idx_train5 = np.load(path+dataset+"/train5.npy")
    idx_train10 = np.load(path+dataset+"/train10.npy")
    ...
    if scope_flag == 1:
        scope = torch.load(path+dataset+"/scope_1.pt")
    if scope_flag == 2:
        scope = torch.load(path+dataset+"/scope_2.pt")
    return adj, features, labels, idx_train, idx_val, idx_test, laplace, scope
